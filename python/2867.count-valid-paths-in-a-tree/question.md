# [2867. 统计树中的合法路径数目][link] (Hard)

[link]: https://leetcode.cn/problems/count-valid-paths-in-a-tree/

给你一棵 `n` 个节点的无向树，节点编号为 `1` 到 `n` 。给你一个整数 `n` 和一个长度为 `n - 1` 的二维整
数数组 `edges` ，其中 `edges[i] = [uᵢ, vᵢ]` 表示节点 `uᵢ` 和 `vᵢ` 在树中有一条边。
...
- 路径 `(a, b)` 指的是一条从节点 `a` 开始到节点 `b` 结束的一个节点序列，序列中的节点 **互不相同** ，
且相邻节点之间在树上有一条边。
